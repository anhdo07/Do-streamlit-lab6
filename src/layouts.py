import pandas as pd
import streamlit as st

from src.charts import plot_response_hist, plot_borough_bar


def header_metrics(df: pd.DataFrame) -> None:
    c1, c2, c3 = st.columns(3)

    # TODO (IN-CLASS): Replace these placeholders with real metrics from df
    # Suggestions:
    # - Total complaints (len(df))
    # - Median response time
    # - % from Web vs Phone vs App (pick one)
    total_complaints = len(df)
    median_value = df['response_time_days'].median() if total_complaints > 0 else 0

    top_complaints_type = df['complaint_type'].value_counts().idxmax()

    with c1:
        st.metric("Total complaints", f"{total_complaints:,}")
    with c2:
        st.metric("Median response (days)", f"{median_value:.1f}")
    with c3:
        st.metric("Most common complaint", top_complaints_type)


def body_layout_tabs(df: pd.DataFrame) -> None:
    """Tabs layout with 3 default tabs."""
    t1, t2, t3 = st.tabs(["Distribution", "By Borough", "Table"])

    with t1:
        st.subheader("Response Time Distribution")
        plot_response_hist(df)

        # TODO (IN-CLASS): Add a short interpretation sentence under the chart
        st.write(f"This histogram shows the distribution of complaints per borough.")

    with t2:
        st.subheader("Median Response Time by Borough")
        plot_borough_bar(df)

        # TODO (IN-CLASS): Add a second view here (e.g., count by borough)
        st.divider()
        st.subheader("Total Calls by Borough")
        borough_count = df['borough'].value_counts()
        st.bar_chart(borough_count)

    with t3:
        st.subheader("Filtered Rows")
        st.dataframe(df, use_container_width=True, height=480)

        # TODO (OPTIONAL): Add st.download_button to export filtered rows
        csv_data = df.to_csv(index=False).encode('utf-8')
        st.download_button(
            label="Download Filtered Data CSV",
            data=csv_data,
            file_name="filtered_data.csv",
            mime="text/csv",
        )
