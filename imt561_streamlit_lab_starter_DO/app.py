...
def main() -> None:
    ...
    # - show row count
    st.write(f"There are {len(df)} rows in NYC 311 data")
    ...
    Design Note: This dashboard helps identify 311 Service Response Efficiency. It is designed for NYC Administors to monitor the response time days metric across different metrics like channels, boroughs, and complaint types. 
    
    Key questions are:
    Which complaint types take the longest to resolve?
    Are there significant delays in specific boroughs that require additional resource allocation?
    Does the channel impact how quickly a ticket is processed?
     """)
    # -------------------------
